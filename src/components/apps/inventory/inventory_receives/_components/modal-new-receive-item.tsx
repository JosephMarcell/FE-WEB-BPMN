...
import { useCreateInventoryReceive } from '@/app/api/hooks/inventory/inventory_receive/useCreateInventoryReceive';
import { useGetInventoryReceiveByPkid } from '@/app/api/hooks/inventory/inventory_receive/useGetInventoryReceiveByPkid';
import { useUpdateInventoryReceive } from '@/app/api/hooks/inventory/inventory_receive/useUpdateInventoryReceive';
...
import {
  inventoryReceiveItemDetailInitialState,
  InventoryReceiveItemDetailProperty,
  inventoryReceiveItemInitialState,
} from '@/helpers/utils/inventory/inventory_receive_item';
...
const ModalNewReceiveItem = ({
  ...
  const [form, setForm] = useState(inventoryReceiveItemInitialState);
  ...
  const { mutateAsync: createInventoryReceive } = useCreateInventoryReceive();
  const { mutateAsync: updateInventoryReceive } = useUpdateInventoryReceive();
  ...
  const {
    data: inventoryReceiveDataDetail,
    isLoading,
    refetch: refetchDetail,
  } = useGetInventoryReceiveByPkid(pkid, enabled);
  ...
  useEffect(() => {
    if (inventoryReceiveDataDetail && modalEdit && !isLoading) {
      setForm(inventoryReceiveDataDetail);
      setReceiveDetail(inventoryReceiveDataDetail.receiveDetails);
    }
  }, [inventoryReceiveDataDetail, modalEdit, isLoading]);
    ...
    if (!isMandatoryEmpty) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            if (modalEdit) {
              const tempForm = { ...form, receiveDetails: receiveDetail };

              await updateInventoryReceive({
                pkid: pkid,
                data: tempForm,
              });
              setModalEdit(false);
            ...
            if (modal) {
              const inputForm = { ...form, receiveDetails: receiveDetail };

              await createInventoryReceive(inputForm);
              setModal(false);
            }
            setForm(inventoryReceiveItemInitialState);
            setEmptyField([]);
  ...
  const handleOnChange = (
    ...
    if (name === 'type') {
      ...
      setReceiveDetail(inventoryReceiveItemDetailInitialState);
  ...
  const handleOnChangeInventoryReceiveDetail = (
    value: string | number | boolean | Date | null,
    key: string,
    index: number,
  ) => {
    setReceiveDetail(
      receiveDetail.map((item, i) =>
                              ...
                              <tr className='border' key={index}>
                                ...
                                <td>
                                  <input
                                    ...
                                    className='form-input'
                                    onChange={e =>
                                      handleOnChangeInventoryReceiveDetail(
