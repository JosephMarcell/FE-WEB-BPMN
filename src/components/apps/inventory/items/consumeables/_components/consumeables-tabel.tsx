...
interface ConsumeablesData extends MyData {
  code: string;
  name: string;
...
interface IProps {
  data?: ConsumeablesData[];
  permission?: string;
  isLoading?: boolean;
  refetch?: () => void;
}

const ConsumeablesTable: React.FC<IProps> = ({ data, isLoading, refetch }) => {
  ...
  const mappedData: ConsumeablesData[] = dataArray.map(item => ({
    ...item,
  ...
  return (
    <RenderDataTable
      ...
      detailPath='/inventory/items/consumeables'
      action='RUD'
    />
  );
};

export default ConsumeablesTable;
