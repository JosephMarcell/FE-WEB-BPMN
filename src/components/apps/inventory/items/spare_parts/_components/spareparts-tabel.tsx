...
interface SparepartsData extends MyData {
  code: string;
  name: string;
...
const SparepartsTable: React.FC<IProps> = ({ data, isLoading, refetch }) => {
  ...
  const mappedData: SparepartsData[] = dataArray.map(item => ({
    ...item,
  ...
  return (
    <RenderDataTable
      ...
      detailPath='/inventory/items/spare_parts'
      action='RUD'
    />
  );
};
