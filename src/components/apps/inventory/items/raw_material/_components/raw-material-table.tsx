...
interface RawMaterialData extends MyData {
  code: string;
  name: string;
...
const RawMaterialTable: React.FC<IProps> = ({ data, isLoading, refetch }) => {
  ...
  const mappedData: RawMaterialData[] = dataArray.map(item => ({
    ...item,
  ...
  return (
    <RenderDataTable
      ...
      detailPath='/inventory/items/raw_material'
      action='RUD'
    />
  );
};
