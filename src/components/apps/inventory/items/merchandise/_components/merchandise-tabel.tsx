...
interface MerchandiseData extends MyData {
  code: string;
  name: string;
...
const MerchandiseTable: React.FC<IProps> = ({ data, isLoading, refetch }) => {
  ...
  const mappedData: MerchandiseData[] = dataArray.map(item => ({
    ...item,
  ...
  return (
    <RenderDataTable
      ...
      detailPath='/inventory/items/merchandise'
      action='RUD'
    />
  );
};
