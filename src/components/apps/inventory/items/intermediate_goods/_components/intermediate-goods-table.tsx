...
interface IntermediateGoodsData extends MyData {
  code: string;
  name: string;
...
interface IProps {
  data?: IntermediateGoodsData[];
  permission?: string;
  isLoading?: boolean;
  refetch?: () => void;
}

const IntermediateGoodsTable: React.FC<IProps> = ({
  data,
  isLoading,
  refetch,
}) => {
  ...
  const mappedData: IntermediateGoodsData[] = dataArray.map(item => ({
    ...item,
  ...
  return (
    <RenderDataTable
      ...
      detailPath='/inventory/items/intermediate_good'
      action='RUD'
    />
  );
};

export default IntermediateGoodsTable;
