...
interface OperationalSuppliesData extends MyData {
  code: string;
  name: string;
...
interface IProps {
  data?: OperationalSuppliesData[];
  permission?: string;
  isLoading?: boolean;
  refetch?: () => void;
}

const OperationalSuppliesTable: React.FC<IProps> = ({
  data,
  isLoading,
  refetch,
}) => {
  ...
  const mappedData: OperationalSuppliesData[] = dataArray.map(item => ({
    ...item,
  ...
  return (
    <RenderDataTable
      ...
      detailPath='/inventory/items/operational_supplies'
      action='RUD'
    />
  );
};

export default OperationalSuppliesTable;
