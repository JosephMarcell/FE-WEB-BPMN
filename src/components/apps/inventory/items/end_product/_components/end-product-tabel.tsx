...
interface EndProductData extends MyData {
  code: string;
  name: string;
...
const EndProductTable: React.FC<IProps> = ({ data, isLoading, refetch }) => {
  ...
  const mappedData: EndProductData[] = dataArray.map(item => ({
    ...item,
  ...
  return (
    <RenderDataTable
      ...
      detailPath='/inventory/items/end_product'
      action='RUD'
    />
  );
};

export default EndProductTable;
