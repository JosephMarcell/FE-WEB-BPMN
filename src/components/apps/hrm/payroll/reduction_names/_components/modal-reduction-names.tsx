...
import { useCreateReductionName } from '@/app/api/hooks/hrm/reduction_name/useCreateReductionName';
import { useGetReductionNameByPkid } from '@/app/api/hooks/hrm/reduction_name/useGetReductionNameByPkid';
import { useUpdateReductionName } from '@/app/api/hooks/hrm/reduction_name/useUpdateReductionName';
import { reductionNameInitialState } from '@/helpers/utils/hrm/reduction_name';
import { variableSalaryTypesInitialState } from '@/helpers/utils/hrm/variable_salary_types';

interface IModalReductionNamesProps {
  modal: boolean;
  modalEdit: boolean;
...
const ModalReductionNames = ({
  modal,
  modalEdit,
  setModal,
  setModalEdit,
  refetch,
}: IModalReductionNamesProps) => {
  const { mutateAsync: createReductionName } = useCreateReductionName();
  const { mutateAsync: updateReductionName } = useUpdateReductionName();
  const pkid = useSelector((state: IRootState) => state.themeConfig.pkid);
  const [form, setForm] = useState(reductionNameInitialState);
  const [emptyField, setEmptyField] = useState([] as string[]);

  const {
    data: reductionNameDetail,
    isLoading,
    refetch: refetchDetail,
  } = useGetReductionNameByPkid(pkid);

  useEffect(() => {
    if (pkid && modalEdit && !isLoading) {
      refetchDetail();
      setForm(reductionNameDetail);
    }
  }, [pkid, modalEdit, reductionNameDetail, isLoading, refetchDetail]);
  ...
  const handleCancel = () => {
    if (
      ...
      setForm(reductionNameInitialState);
      setEmptyField([]);
    } else {
      Swal.fire({
        title: 'Are you sure?',
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            ...
            if (modal) {
              await createReductionName(form);
              setModal(false);
            }
            setForm(variableSalaryTypesInitialState);
            setEmptyField([]);
            Swal.fire(
              'Saved!',
              'Your reduction name has been saved.',
              'success',
            ).then(() => {
              refetch();
            });
            ...
            <Dialog.Panel className='panel animate__animated animate__slideInDown dark:text-white-dark my-8 w-full max-w-6xl overflow-hidden rounded-lg border-0 p-0 text-black'>
              <div className='flex items-center justify-between bg-[#fbfbfb] px-5 py-3 dark:bg-[#121c2c]'>
                <h5 className='text-lg font-bold'>
                  {modalEdit ? 'Edit ' : 'New '}Reduction Name
                </h5>
                <button
                  onClick={handleClose}
                  type='button'
...
export default ModalReductionNames;
